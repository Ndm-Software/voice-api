...
export class VoiceCallProcessor {
  private readonly logger =
    new Logger(VoiceCallProcessor.name);
  ...
  async handleVoiceCall(
    ...
    this.logger.log(
      `Voice call job başladı. Job ID: ${job.id}`,
    );
    ...
    const voiceSetting =
      reminder.voiceCallSettings.find(
        (setting) =>
          setting.callId === job.data.settingId,
      );
...
this.logger.log(`Twilio hedef numara: ${phoneNumber}`);
   const result = await this.voiceCallService.makeCall(phoneNumber);
